...
\end{equation}
For this method $n$ is a structural parameter and $\bm{\lambda} = n$. The example of approximation using autoregressive model is demonstrated on the Figure~\ref{fig::ar_example}.
...
This approach gives a different approximation compared to autoregressive model, one can find it in the Figure~\ref{fig::spline_example}. In the Figure~\ref{fig::ar_example} and in the Figure~\ref{fig::spline_example} we approximate the same time series, so one can notice the main distinction of different methods.
...
\begin{equation*}
	\bm{\hat{\theta}} = \argmin_{\mathbb{\theta}} L(\bm{\theta}) 
\end{equation*}

We make a prediction for a new object in a following way:
\begin{equation*}
	\hat{y} = \sgn (\bm{\hat{\theta}}^T \mathbf{h})
\end{equation*}
...
\begin{align*}
	\bm{\hat{\theta}}  = \begin{pmatrix}
	\mathbf{\hat{w}}\\ \hat{b}\\ \bm{\hat{\xi}}
...
The results of the experiments for the both datasets is presented in Figure~\ref{fig::accuracy_results}. All results with classification accuracy scores of each class are represented in Table~\ref{tbl::wisdm_methods_results} for WISDM dataset and in Table~\ref{tbl::uschad_methods_results} for USCHAD.

According to the Figure~\ref{fig::accuracy_results} for WISDM dataset the worst result is obtained by splines parameters. The results for expert functions, autoregressive model and SSA is roughly identical. For USC-HAD dataset the results is highly depend on the classification model. For both datasets logistic regression shows the worst quality, while the accuracy for support vector machine and random forest are strongly correlated.
...
\begin{table}[h]
	\centering
	\caption{Accuracy scores of classification of each class using different methods of feature generation: EX~--- Expert, AR~--- Auto-Reg, SSA and  SPL for Splines}
	\footnotesize
