...
The research consider accelerometer time series as complex structured objects. Time series is represented as the vector with fixed length $T$:
\begin{equation}
...
Our assumption is that the target function $f^*$ could be approximated by some function $\hat{f}$ from the class of function compositions $f = g \circ \bm{h}$. 
...
The research consider different local approximation models as the feature generation methods. 
...
\begin{equation*}
\textrm{accuracy} = \frac{1}{m} \sum_{i=1}^{m} [y_i = \hat{y}_i],
\end{equation*}
...
Each component of the object $s$ from~\eqref{eq::time_series} is assumed as a linear combination of the previous $n$ components 
\begin{equation*}
...
The feature map $\hat{\bm{h}}(s)$ is given by optimal parameters of autoregressive model $\mathbf{w}^* = \{w^*_j\}_{j=0}^n$ for time series $s$. 
The optimal parameters are defined by minimizing the squared error between the original object $s$ and its prediction of the model~\eqref{eq::autoregression_prediction}
...
\end{equation}
The problem~\eqref{eq::autoregression_description} is the linear regression problem. Hence, for each initial time series $s$ we have to solve linear regression problem with $n$ predictors.
...
\begin{align*}
	...
	\cdots &\\
	p_{M}(x) = w_{L0} +w_{L1}t + w_{L2}t^2 + w_{L3}t^3, & t\in [\xi_{M-1}, \xi_M]					
	\end{cases}\\
...
{\color{red} ПОЧЕМУ ТУТ НЕТ СВОБОДНОГО ЧЛЕНА b, А В SVM ЕСТЬ?}
The first approach to classification is regularized logistic regression model. The optimal model parameters~\eqref{eq::optimal_classification_params} is determined by minimising the following error function

\begin{equation*}
	L(\bm{\theta}, \mathfrak{D}_H, \mu) = \sum_{i=1}^{m} \log\left(1 + \exp(-y_i \bm{\theta}^{\T} \mathbf{h}_i)\right) + \frac{\mu}{2} \|\bm{\theta}\|^2,
\end{equation*}
...
\begin{equation*}
	\hat{y} = g(\mathbf{h}, \bm{\theta}^*) = \sgn(\mathbf{h}^{\T} \bm{\theta}^*).
\end{equation*}
...
The external structural parameters procedures,  such as the length $n$ for autoregression, the window width $n$ for SSA and the number of splines knots $L$, were tuned using 3-fold cross validation procedure. {\color{red} ФОРМУЛА CV}
...
The autoregressive model were tuned to find the optimal length $n$. Cross validation procedure gives optimal value $n=20$ for both dataset. 

The singular spectrum analysis were tuned in the same way to find the optimal window width $n$. Analogously to autoregressive model the cross validation procedure gives the same value $n=20$. Authors assume that it means that time series has memory of this size.

We fit splines for time series using $scipy$ python library {\color{red} ССЫЛКА НА SCIPY}. 
...
The number~$M$ were chosen implicitly by choosing the proper smoothing parameter~$s$. 
The less the value of $s$, the larger the number of knots $M$. 
...
The next step is finding the smoothing parameter in this interval using bi-search approach. {\color{red} ВОТ ЭТО БЫЛ СТРАННЫЙ АБЗАЦ}
