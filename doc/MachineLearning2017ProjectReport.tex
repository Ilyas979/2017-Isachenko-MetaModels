...
\begin{itemize}
	...
	These functions can be expressed as a set of statistics $\{h_i\}_{i=1}^n$.
	...
	\begin{equation}
		\widehat{\mathbf{h}} = \arg \min_{\mathbf{h}} S( s, \mathbf{h}, \bm{\lambda}).
...
In our project we consider accelerometer time series as a complex objects. Time series is represented in following way:
\[
	s_i = \{x_1, \cdots, x_T\},
\]
where $T$ denotes the length of time series. Now let us expand on different approaches of feature generation.

\section*{Feature generation approaches}
\subsection*{Expert functions}
Given a set of complex objects $\{s_i\}_{i=1}^m$ we can extract features in a non-parametric way with a set of expert functions $\{h_i\}_{i=1}^n$. For time series, these functions could be mean, standard deviations, mean average deviations ans distributions of the values. The main drawback of this approach is that we are restricted by our choice of expert functions and for some types of data these functions might be impossible to derive.

\subsection*{Model of autoregression}
In this method we assume autoregressive model of the order $n$ as a hypothesis for generation of time series $s$. Here, $n$ is a structural parameter and $\bm{\lambda} = n$. We have
\begin{equation*}
	x_t = \theta_0 + \sum_{j=1}^{n} \theta_i x_{t-j} + \epsilon_t.
\end{equation*}
Optimal parameters of autoregressive model $\bm{\hat{\theta}}_i = \mathbf{h}_i$ for time series $s_i$ are found by minimizing mean squared error:
\begin{equation*}
	\mathbf{h}_i = \argmin_{\bm{\theta} \in \Theta} S(s_i, \bm{\theta}, \bm{\lambda}) = \argmin_{\bm{\theta} \in \Theta} \big( \sum_{t=1}^{T} \|x_i - \hat{x}_i\|\big),
\end{equation*}
where
\begin{equation*}
	\hat{x}_i = \theta_0 + \sum_{j=1}^{n} \theta_i x_{t-j}.
\end{equation*}
