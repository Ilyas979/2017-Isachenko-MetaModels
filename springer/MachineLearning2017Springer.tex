...
\title{Feature Generation for Physical Activity Classification\thanks{This research is supported by RFBR project 16-07-01154}}
%
\titlerunning{Feature Generation}  % abbreviated title (for running head)
%                                     also used for the TOC unless
%                                     \toctitle is used
%
\author{Roman Isachenko\inst{1}, Ilya Zharikov\inst{1}, 
	Artem Bochkarev\inst{1}, Vadim Strijov\inst{2}}
%
...
%
\institute{Moscow Institute of Physics and Technology, Dolgoprudny, 141700, Russia \\ 
...
\begin{abstract}
	The development of wearable devices make it possible to analyse human physical activity.
	The paper is devoted to the problem of classification accelerometer time series of smart phones.
	The problem is to generate set of relevant features. Parameters of local approximation models is used as input features for classifier. 
	We compare several approaches for feature extraction such as expertly defined features, autoregressive model and SSA. 
	We propose new method for feature generation, based on local spline approximation. 
	The experiment is conducted on two datasets for human activity recognition using accelerometer.
	\keywords{complex structured objects, time series, local approximation models}
\end{abstract}

\section{Introduction}
The paper investigates the multiclass classification problem of objects which don't have explicit feature representation. This problem arises in analysing biological data~\cite{motrenko2016extracting}, human behavior and social interactions~\cite{bao2004activity}. We consider the problem of human activity recognition.
The accelerometer time series~\cite{ignatov2016human,lu2016towards,wang2014human} from smart phones serve to recognize human physical activity in the internet of things~\cite{budnik2016learned,lu2016activity}.
...
First, we extract informative features, and then use those features as input to some classifier to obtain final model. 
For simplicity, we assume that these two procedures can be built and analysed separately. 
In our project we focus mainly on comparing different methods of feature generation~\cite{karasikov2016feature,ivkin2015ts} such as expert-defined functions, autoregressive model and singular spectrum analysis.
Expert-defined functions~\cite{kwapisz2011activity} allow to avoid the problem of feature extraction. These functions for time series task include average value, standard deviation, mean absolute deviation and distribution for each component. In autoregressive model~\cite{lukashin2003adaptive} we build parametric model for each time series and use parameters of the model as features for classification. Singular spectrum analysis~\cite{hassani2007singular} proposes to use eigenvalues of trajectory matrix as features for building classifier.

The authors propose a new feature generation method. 
We approximate time series with cubic splines~\cite{deboor1978splines}. 
The spline interpolates the 3-order piecewise curve at the given dots. The additional smoothess conditions makes the curve and its first and second derivatives continuous. Splines give the smooth curve and adequate quality of approximation.
...
\end{equation}
We assume that there is a hidden true dependence $a^*: \mathcal{S} \rightarrow Y$ between objects from the space $\mathcal{S}$ and their class labels from $Y$.
Denote by $\mathcal{D} = \{(s_i, y_i)\}_{i=1}^m$ a~given sample, where $s_i \in \mathcal{S}$ and $y_i = a^*(s_i)\in Y$.
The problem is to recover the function~$a^*$. 
We assume that the target function $a^*$ can be approximated by some function from the class of function compositions $a = f \circ \bm{g}$. 
Here $\bm{g}: \mathcal{S} \rightarrow G$ is a map from the original space $\mathcal{S}$ to the feature space $G \subset \bbbr^n$, and
$$f: G \times \Theta \rightarrow Y$$ is a parametric map from the feature space $G$ to the set of class labels $Y$. 
The function $f$ corresponds to classification model which is parametrized by a vector parameter $\boldsymbol{\theta} \in \Theta$. 

The determining of the approximation function $a$ is equivalent to determining the optimal functions $\bm{g}$ and  $f$. 
The function $\bm{g}$ corresponds to generating the appropriate feature space $G$. 
...
Given appropriate feature space $G$ and feature map $\bm{g}$ we transform our original sample $\mathcal{D} = \{s_i, y_i\}_{i=1}^m$ with complex structured objects to the new sample $\mathcal{D}_G = \{\bm{g}_i, y_i\}_{i=1}^m$, where $\bm{g}_i = \bm{g}(s_i) \in G$. 
The function $h(\bm{g}, \bm{\theta})$ is defined by its parameter vector $\bm{\theta} \in \Theta$. 
...
\end{equation}
where the function $L(\bm{\theta}, \mathcal{D}_G, \bm{\mu})$ is the classification error function~\eqref{eq::accuracy}. Here the vector $\bm{\mu}$ is a external parameters of the particular classification model. Examples of these parameters for different classification models are given below.
...
Accuracy score is correctly classified objects and their total number in dataset:
\begin{equation}
\mathrm{accuracy} = \frac{1}{m} \sum_{i=1}^{m} [y_i = \hat{y}_i],
\label{eq::accuracy}
\end{equation}
where $\hat{y}_i = \hat{z}(s_i)$ is a prediction of the classifier. 
...
These functions are statistics $g_j$, where $g_j: \mathcal{S} \rightarrow \bbbr$.
...
The optimal parameters $\mathbf{w}^*$ of the autoregressive model are the features $\bm{g}(s)$.
These parameters minimize the squared error between the time series $s$ and its prediction

\begin{equation}
\bm{g}(s) = \mathbf{w}^* = \argmin_{\mathbf{w} \in \bbbr^{n}} \left( \sum_{t=n}^{T} \|x_t - \hat{x}_t\|^2\right).
...
\[
\bm{g}(s) = (\mathbf{w}_1, \dots, \mathbf{w}_{M}).
\]
...
\end{bmatrix}.
\end{equation*}
Thus, the optimal parameters is given by~\eqref{eq::optimal_classification_params}.
...
The results show that obtained features allows to recover the class label with the high quality. The proposed spline approximation method gives the quality competitive with existing stated methods.
